...
export const BlueAirDeviceSensorDataMap: Record<
  string,
  keyof BlueAirDeviceSensorData
> = {
  fsp0: "fanspeed",
  hcho: "hcho",
  h: "humidity",
...
export default class BlueAirAwsApi {
  ...
  constructor(
    ...
    this.logger
      .debug(`Creating BlueAir API instance with config: ${JSON.stringify(config)} and username: ${username}\
    and region: ${region}`);
  ...
  async getDevices(): Promise<BlueAirDeviceDiscovery[]> {
    ...
    this.logger.debug(
      `Device discovery response: ${JSON.stringify(devices.map((d) => ({ name: d.name, type: d.type, uuid: d.uuid })))}`,
    );
    return devices;
  }
